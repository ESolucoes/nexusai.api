...
import { IsIn, IsOptional, IsString, IsUUID, Length, IsDateString, IsUrl, IsNumber } from 'class-validator';
...
export class PostMentoradoDto {
  ...
  @ApiProperty({ required: false }) @IsOptional() @IsString() @Length(0, 120) complemento?: string;
  ...
  @ApiProperty() @IsUrl() linkedin: string;
}
