...
export class AutenticacaoService {
  ...
  private async getUsuarioByEmail(email: string) {
    try {
      ...
      // Método 1: Tenta usar o serviço de usuários
      if (typeof (this.usuariosService as any).findByEmail === 'function') {
        const usuario = await (this.usuariosService as any).findByEmail(
          normalizedEmail,
        );
        console.log('✅ Usuário encontrado via serviço:', usuario?.id);
        return usuario;
      }

      // Método 2: Acesso direto ao repositório
      if ((this.usuariosService as any)['usuariosRepo']) {
        const repo = (this.usuariosService as any)['usuariosRepo'];
        const usuario = await repo.findOne({
          where: { email: normalizedEmail },
        });
        console.log('✅ Usuário encontrado via repositório:', usuario?.id);
        return usuario;
      }

      // Método 3: Busca alternativa
      console.log('⚠️  Usando método alternativo de busca');
      const usuarios = await (this.usuariosService as any).findAll();
      const usuario = usuarios.find(
        (u: any) => u.email.toLowerCase() === normalizedEmail,
      );
      console.log('✅ Usuário encontrado via busca alternativa:', usuario?.id);
