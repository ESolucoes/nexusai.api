...
async function saveDebug(page: Page, name: string) {
  try {
    const ts = Date.now();
    await page.screenshot({ path: path.join(DEBUG_DIR, `${name}-${ts}.png`), fullPage: true }).catch(() => {});
...
async function clickByTexts(page: Page, texts: string[]) {
  ...
  const handles = await page.$$('button, a, input[type="button"], input[type="submit"], div[role="button"]');
  ...
  for (const h of handles) {
    try {
      ...
      const txt = (inner + " " + aria + " " + title + " " + value).toString().toLowerCase();
    ...
    for (const url of filtered.slice(0, maxApply)) {
      try {
        ...
        if (submitted) {
          ...
          console.log(`[PARTIAL] formulário complexo ou submit não detectado: ${url}`);
