...
export class ArquivosService {
  ...
  buildStorageKey(relativePath: string, filename: string) {
    return `${relativePath}/${filename}`.replace(/\\/g, '/').replace(/^\/+/, '')
  }
  ...
  buildPublicUrl(storageKey: string, opts?: { absolute?: boolean; req?: Request }) {
  ...
  buildPrivateUrl(storageKey: string, opts?: { absolute?: boolean; req?: Request }) {
    const key = storageKey.replace(/^\/+/, '').replace(/\\/g, '/')
